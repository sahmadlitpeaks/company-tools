...
export default function CrmPage() {
  ...
  // Built from the data rather than a fixed list: a hardcoded array silently
  // hides whole sources (website leads were unfilterable this way).
  ...
  return (
    <div>
      ...
      <Card className="mb-4"><CardContent className="grid items-end gap-4 sm:grid-cols-5"><Field className="sm:col-span-3"><FieldLabel htmlFor="crm-search">Search</FieldLabel><Input id="crm-search" placeholder="Name, email, company…" value={q} onChange={(event) => setQ(event.target.value)} /></Field><Field><FieldLabel htmlFor="crm-filter-status">Status</FieldLabel><Select items={[{ value: null, label: "All" }, ...STATUSES.map((value) => ({ value, label: value }))]} value={status || null} onValueChange={(value) => setStatus(value ?? "")}><SelectTrigger className="w-full" id="crm-filter-status"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value={null}>All</SelectItem>{STATUSES.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectGroup></SelectContent></Select></Field><Field><FieldLabel htmlFor="crm-source">Source</FieldLabel><Select items={[{ value: null, label: "All" }, ...sources.map((value) => ({ value, label: SOURCE_LABELS[value] ?? value }))]} value={source || null} onValueChange={(value) => setSource(value ?? "")}><SelectTrigger className="w-full" id="crm-source"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value={null}>All</SelectItem>{sources.map((value) => <SelectItem key={value} value={value}>{SOURCE_LABELS[value] ?? value}</SelectItem>)}</SelectGroup></SelectContent></Select></Field></CardContent></Card>
